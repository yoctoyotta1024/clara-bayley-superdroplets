...
def dimless_randomexpo_dist(nsupers, parcel_vol, n_a, r_a, rho=1):
  ...
  eps = np.full(nsupers, n_a*parcel_vol/nsupers)
...
if use_lognormal:
  ...
  r0, eps = dimless_randomexpo_dist(nsupers, parcel_vol, n_a, r_a)
  m_sol = Rho_sol*4/3*np.pi*r0**3                   # assuming initially dry areosol droplets (dimless mass_solute)
