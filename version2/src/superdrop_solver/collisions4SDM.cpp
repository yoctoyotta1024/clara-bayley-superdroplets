...
void collide_droplets(const int nsupers,
                      const int nhalf, vector<int> pvec,
  ...
  all superdroplets in given volume */
{
  ...
  /* Neccesary for random permutation of
    indicies vector (pvec) and number generators */
  random_device rd;
  ...
  /* Randomly shuffle order of indicies of
    droplets in order to generate random pairs */
  shuffle(pvec.begin(), pvec.end(), seed);

  /* This loop does Monte-Carlo collision
    of each random pair of superdroplets */
  for (int i = 0; i < nhalf; i++)
  {
    ...
    prob = scale_p * max(drop1.eps, drop2.eps) * prob_jk; // scaled probability of pair coalescence (p_alpha in Shima et al. 2009)

    /* 3. Monte Carlo Step: randomly 
      determine gamma of coalescence */
    ...
    gamma = min(gamma, maxgamma); // gamma factor Shima et al. 2009

    /* 4. coalesce particles if gamma != 0 */
    if (gamma != 0)
    {
      /* 5.(option b) if eps1 = gamma*eps2 collide to
      make twin SDs with same eps, r and mass */

      if (drop1.eps == gamma * (drop2.eps))
      {

        delta_eps = (drop2.eps) / 2.0;
        delta_r = pow((drop2.r), 3.0) + gamma * (pow((drop1.r), 3.0));
        delta_r = pow(delta_r, 1.0 / 3.0);
      ...
      /* 5.(option a) if eps1 > gamma*eps2 collide to grow
      ...
      /* 5.(option "c") this should not be an option => throw error warning */
      else
      {
